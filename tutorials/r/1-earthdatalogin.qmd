...
```{r}
earthdatalogin::edl_netrc() # will use a default example log in
```
...
If that doesn't work, it is likely because you do not have the  End User Licence Agreement (EULA)/permissions to use that dataset or are properly logged in. Try this example script from the `?earthdatalogin::edl_netrc` documentation: 
