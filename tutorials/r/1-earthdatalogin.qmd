...
This will put your login info in a  `netrc` located at:
...
Once your netrc file is saved, you can use
```{r}
earthdatalogin::edl_netrc()
```
to authenticate.

*For the purposes of this workshop, `edl_netrc()` will work but it will use a default public account. Don't use that if you have a NASA Earthdata account.*
...
If that doesn't work, it is likely because you do not have the  End User Licence Agreement (EULA)/permissions to use that dataset or are not properly logged in. Try this example script from the `?earthdatalogin::edl_netrc` documentation: 
